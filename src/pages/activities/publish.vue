<template>
	<view class="container">
		...
		<!-- 标签栏 -->
		<view class="tabs">
			<view class="tab" :class="{active: activeTab === 'basic'}" @click="switchTab('basic')">基本资料</view>
			<view class="tab" :class="{active: activeTab === 'detail'}" @click="switchTab('detail')">详细介绍</view>
		</view>
			...
			<view v-if="activeTab === 'basic'" class="form-container">
				...
				<view class="form-item">
					...
					<view class="input-wrapper" @click="showLocationPicker">
						<text class="input" :class="{placeholder: !activityData.location}">
		...
		<view class="save-section">
			<button class="save-btn" :class="{saved: isSaved, 'publish-style': activeTab === 'detail', 'published': isPublished}" @click="handleButtonClick" :disabled="(isSaved && activeTab === 'basic') || isPublished">
			...
			chooseImage() {
				...
				uni.chooseImage({
					...
					success: (res) => {
						...
						// 将选择的图片添加到数组中
						this.activityData.images = this.activityData.images.concat(res.tempFilePaths);
						uni.showToast({
